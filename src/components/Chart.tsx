...
const Chart: React.FC<ChartProps> = ({ tokenAddress, symbol, marketCap, onRaydium }) => {
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between text-white p-2 gap-2 sm:flex-nowrap">
        <div className="flex space-x-2">
          ...
          <span className="text-xs sm:text-sm">
            <strong>MCap:</strong> ${priceData.marketCapN.toLocaleString('en-US')}
          </span>
        </div>
        ...
        <div className="flex space-x-4">
          ...
          {onRaydium ?
            <a
              href={`https://raydium.io/swap/?inputMint=sol&outputMint=${tokenAddress}`}
              target="_blank"
              rel="noopener noreferrer"
              className="text-xs sm:text-sm flex items-center space-x-1"
            >
      ...
      {/* Chart Container */}
      <div className={`w-full rounded-xl  bg-gray-900 shadow-xl`}
      // {ref={chartContainerRef}} 
      >
        <iframe
          width="100%"
          height="600"
          src={`https://birdeye.so/tv-widget/${tokenAddress}?chain=solana&viewMode=pair&chartInterval=15&chartType=Candle&chartTimezone=Asia%2FSingapore&chartLeftToolbar=show&theme=dark`}
          frameBorder="0"
          allowFullScreen
        ></iframe>
      </div>
